<template>
  <div class="max-w-4xl mx-auto px-4 py-6">
    <div class="flex items-center justify-between flex-wrap gap-4 mb-6">
      ...
      <h1 class="text-2xl font-bold text-gray-800">Создать урок</h1>
    </div>
    ...
    <form @submit.prevent="submit" class="space-y-4 bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
      ...
      <div>
        ...
        <input
          ...
          type="text"
          placeholder="Например: Техника, Меню"
          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary"
        />
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Содержание урока *</label>
        <!-- Расширенный тулбар: жирный, курсив, заголовки, списки, размер шрифта, изображения -->
        <div
          v-if="editor"
        ...
        >
          ...
          <button
            type="button"
            class="px-2 py-1.5 rounded text-sm font-medium"
            :class="editor.isActive('heading', { level: 2 }) ? 'bg-primary/20 text-primary' : 'text-gray-600 hover:bg-gray-200'"
          ...
          <button
            type="button"
            class="px-2 py-1.5 rounded text-sm font-medium"
            :class="editor.isActive('heading', { level: 3 }) ? 'bg-primary/20 text-primary' : 'text-gray-600 hover:bg-gray-200'"
          ...
          <button
            type="button"
            class="px-2 py-1.5 rounded text-sm font-medium"
            :class="editor.isActive('bulletList') ? 'bg-primary/20 text-primary' : 'text-gray-600 hover:bg-gray-200'"
          ...
          <button
            type="button"
            class="px-2 py-1.5 rounded text-sm font-medium"
            :class="editor.isActive('orderedList') ? 'bg-primary/20 text-primary' : 'text-gray-600 hover:bg-gray-200'"
          ...
          <span class="w-px bg-gray-300 self-stretch my-1" />
          <select
            class="px-2 py-1.5 rounded text-sm border border-gray-300 bg-white text-gray-700 focus:ring-2 focus:ring-primary/20"
            title="Размер шрифта"
        ...
        <div
          class="min-h-[280px] border border-gray-300 rounded-b-lg px-3 py-2 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary prose prose-sm max-w-none"
          :class="editor ? '' : 'rounded-t-lg'"
        >
          <editor-content :editor="editor" class="lesson-editor" />
        </div>
        <p class="mt-1 text-xs text-gray-500">
          Заголовки H2/H3, маркированный и нумерованный списки, размер шрифта, изображения.
        </p>
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Связанный тест (необязательно)</label>
        <select
          v-model="form.testId"
          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/20 bg-white"
        >
...
const editor = useEditor({
  ...
  extensions: [
    ...
    Placeholder.configure({
      placeholder: 'Напишите содержание урока. Используйте заголовки, списки и изображения для наглядности.',
    }),
  ],
  editorProps: {
    attributes: {
      class: 'outline-none min-h-[220px]',
    },
  },
})
...
async function submit() {
  ...
  if (!form.title.trim() || !textOnly) {
    error.value = 'Заполните заголовок и содержание урока.'
    return
  }
  error.value = ''
  success.value = ''
  saving.value = true
  try {
    const res = await api.guru.createLesson({
    ...
    success.value = 'Урок создан.'
    router.push({ name: 'GuruLessonView', params: { id: res.id } })
  } catch (e) {
    error.value = e.data?.message || e.message || 'Ошибка сохранения урока'
  } finally {
    saving.value = false
  }
}
...
.lesson-editor .tiptap img {
  max-width: 100%;
  height: auto;
  border-radius: 0.5rem;
  margin: 1rem 0;
}
.lesson-editor .tiptap ul {
  list-style-type: disc;
  padding-left: 1.5rem;
  margin: 0.5rem 0;
}
.lesson-editor .tiptap ol {
  list-style-type: decimal;
  padding-left: 1.5rem;
  margin: 0.5rem 0;
}
.lesson-editor .tiptap h2 {
  font-size: 1.25rem;
  font-weight: 600;
  margin-top: 1rem;
  margin-bottom: 0.5rem;
}
.lesson-editor .tiptap h3 {
  font-size: 1.125rem;
  font-weight: 600;
  margin-top: 0.75rem;
  margin-bottom: 0.5rem;
