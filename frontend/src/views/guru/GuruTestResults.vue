<template>
  <div class="max-w-4xl mx-auto px-4 py-6">
    ...
    <template v-else>
      <h1 class="text-2xl font-bold text-gray-800 mb-4">Результаты по тесту «{{ testTitle }}»</h1>
          ...
          <table class="w-full text-left">
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th class="px-4 py-3 text-sm font-medium text-gray-600">Сдан</th>
            ...
            <tbody>
              ...
              >
                <td class="px-4 py-3 text-gray-800">{{ r.userName }}</td>
                <td class="px-4 py-3 text-gray-800">{{ r.score }}%</td>
                <td class="px-4 py-3 text-gray-800">{{ r.pointsEarned }} / {{ r.pointsMax }}</td>
                <td class="px-4 py-3 text-gray-800">{{ r.timeSpent != null ? formatTime(r.timeSpent) : '—' }}</td>
                <td class="px-4 py-3">{{ r.passed ? 'Да' : 'Нет' }}</td>
                <td class="px-4 py-3 text-gray-600 text-sm">{{ formatDate(r.finishedAt) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <p v-if="results.length === 0" class="p-6 text-gray-500 text-center">Нет прохождений.</p>
      </div>
      <router-link
...
onMounted(async () => {
  try {
    ...
    const res = await api.guru.testResults(testId)
